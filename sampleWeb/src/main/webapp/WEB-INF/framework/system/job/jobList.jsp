...
<script type="text/javascript">
	...
	function update(){
		 ...
		 if (ids.length == 0) {
        	 ...
        	 commonForward('${path}/sys/job/form?id='+rowdata.id);
         }
	}
	function updateJobStatus(status){
		 ...
		 if (ids.length == 0) {
        	...
        	if(confirm(msg)){
        		var rowdata = $("#jobTable").jqGrid("getRowData", ids[0]);
	        	commonForward('${path}/sys/job/updateJobStatus?id='+rowdata.id+'&jobStatus='+status);
       	 	}
        }
	}
	function batchDelete(){
		 ...
		 if (ids.length == 0) {
	       	 ...
	       	 if(confirm("确定进行批量删除？")){
	        	 commonForward('${path}/sys/job/delete?ids='+ids.toString());
	       	 }
	     }
	}
</script>
	...
	<div class="page-content" id="mainContent">
		<div class="widget-box transparent ui-sortable-handle collapsed"
			...
			<div class="btn-group">
				...
				<button type="button" class="btn btn-white btn-sm btn-primary" onclick="commonForward('${path}/sys/job/form')">新增</button>
			...
			<yqjr:table id="jobTable" colModel="[{label:'ID',name:'id',index:'id',sortable:true, width:150, align:'left',hidden:true},
					    ...
					    url="${path }/sys/job/listData" shrinkToFit="true"/>
		</div>
	</div>
</body>
...
<script type="text/javascript">
	...
	function update(){
		 ...
		 if (ids.length == 0) {
        	 ...
        	 commonForward('${path}/sys/user/form?id='+rowdata.id);
         }
	}
	function batchDelete(){
		 ...
		 if (ids.length == 0) {
        	 ...
        	 if(confirm("确定进行批量删除？")){
	        	 commonForward('${path}/sys/user/delete?ids='+ids.toString());
        	 }
         }
	}
</script>
		...
		<div class="widget-box transparent ui-sortable-handle collapsed"
			...
			<div class="widget-body">
				<div class="widget-main padding-6 no-padding-left no-padding-right">
					...
					<form:form id="searchForm" modelAttribute="userCondition" method="post" class="form-horizontal">
						<div class="row">
							<div class="col-sm-4">
								<label class="col-sm-4 control-label no-padding-right" for="loginName"> 登录名 ：</label>
								<div class="col-sm-8">
									<form:input path="loginName" htmlEscape="false" maxlength="50" cssClass="form-control input-sm" />
								</div>
							</div>
							<div class="col-sm-4">
								<label class="col-sm-4 control-label no-padding-right" for="status"> 状态 ： </label>
								<div class="col-sm-8">
									<form:select path="deleteStatus" cssClass="form-control input-sm">
										<form:option value="" label="请选择..." />
										<form:options items="${fns:getDictList('data_status') }" itemLabel="label" itemValue="value" htmlEscape="false" cssClass="required"/>
									</form:select>
								</div>
							</div>
							<div class="col-sm-4">
								<div class="dataTables_paginate paging_simple_numbers">
									<ul class="pagination">
										<li>
											<button id="searchBtn" onclick="searchData('userTable')" type="button" class="btn btn-primary">
			...
			<div class="btn-group">
				<button type="button" class="btn btn-white btn-sm btn-primary" onclick="commonForward('${path}/sys/user/form')">用户新增</button>
			...
			<yqjr:table id="userTable" colModel="[     {label:'ID',name:'id',index:'id',sortable:true, width:150, align:'left',hidden:true},
					    ...
					    url="${path }/sys/user/listData" shrinkToFit="true"/>
		</div>
	</div>
</body>
...
(function($ , undefined) {
	...
	var Widget_Box = function(box, options) {
		...
		this.toggle = function(type, button, expandSpeed, collapseSpeed) {
			...
			if( event_name == 'show' ) {
				if($icon) $icon.removeClass($icon_down).addClass($icon_up).html("<span style=\"font-family:'微软雅黑';font-size:14px;\">&nbsp;关闭</span>");
				...
				if($icon) $icon.removeClass($icon_up).addClass($icon_down).html("<span style=\"font-family:'微软雅黑';font-size:14px;\">&nbsp;打开</span>");

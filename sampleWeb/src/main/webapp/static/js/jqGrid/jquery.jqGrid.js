	...
	return this.each( function() {
		...
		var ts= this, grid={
			...
			populateVisible: function() {
				...
				var div = rh * rn;
				var pageNo, npage, empty;
				if ( tbot < dh && ttop <= 0 &&
					(p.lastpage===undefined||(parseInt((tbot + scrollTop + div - 1) / div,10) || 0) <= p.lastpage))
				{
					npage = parseInt((dh - tbot + div - 1) / div,10) || 1;
					if (tbot >= 0 || npage < 2 || p.scroll === true) {
						pageNo = ( Math.round((tbot + scrollTop) / div) || 0) + 1;
						ttop = -1;
					} else {
						ttop = 1;
					}
				...
				if (ttop > 0) {
					pageNo = ( parseInt(scrollTop / div,10) || 0 ) + 1;
					npage = (parseInt((scrollTop + dh) / div,10) || 0) + 2 - pageNo;
					empty = true;
				}
				if (npage) {
					if (p.lastpage && (pageNo > p.lastpage || p.lastpage===1 || (pageNo === p.pageNo && pageNo===p.lastpage)) ) {
						return;
					}
					...
					if(p.scrollPopUp && p.lastpage != null) {
						$("#scroll_g"+p.id).show().html( $.jgrid.template( $.jgrid.getRegional(ts, "defaults.pgtext", p.pgtext) , p.pageNo, p.lastpage)).css({ "top": p.scrollTopOffset+scrollTop*((parseInt(p.height,10) - 45)/ (parseInt(rh,10)*parseInt(p.records,10))) +"px", "left" : p.scrollLeftOffset});
		...
		addXmlData = function (xml, rcnt, more, adjust) {
			...
			ts.p.userData = {};
			ts.p.pageNo = intNum($.jgrid.getXmlData(xml, xmlRd.pageNo), ts.p.pageNo);
			...
			if (gl > 0 &&  ts.p.pageNo <= 0) { ts.p.pageNo = 1; }
			if(gxml && gl){
				...
				while (j<gl) {
					...
					rowData.push("");
					if( ni ) {
						rowData.push( addRowNum(0, j, ts.p.pageNo, ts.p.rowNum, rnc ) );
					}
					if( gi ) {
			...
			if(ts.p.gridview === true) {
				...
				if(ts.p.grouping) {
					if(!locdata) {
						self.jqGrid('groupingRender',grpdata,ts.p.colModel.length, ts.p.pageNo, rn);
						grpdata = null;
					}
		...
		addJSONData = function(data, rcnt, more, adjust) {
			...
			var self = $(ts), ir=0,v,i,j,f=[],cur,gi=ts.p.multiselect?1:0,si=ts.p.subGrid===true?1:0,addSubGridCell,ni=ts.p.rownumbers===true?1:0,arrayReader=orderedCols(gi+si+ni),objectReader=reader(frd),rowReader,len,drows,idn,rd={}, fpos, idr,rowData=[],cn=(ts.p.altRows === true) ? ts.p.altclass:"",
					cn1 = getstyle(stylemodule, 'rowBox', true, 'jqgrow ui-row-'+ ts.p.direction), classes;
			ts.p.pageNo = intNum($.jgrid.getAccessor(data,dReader.pageNo), ts.p.pageNo);
			...
			if (len > 0 && ts.p.pageNo <= 0) { ts.p.pageNo = 1; }
			...
			while (i<len) {
				...
				rowData.push("");
				if( ni ) {
					rowData.push( addRowNum(0, i, ts.p.pageNo, ts.p.rowNum, rnc ) );
				}
				if( gi ){
		...
		addLocalData = function() {
			...
			pageNo = parseInt(ts.p.pageNo,10),
			...
			if(ts.p.treeGrid && ts.p.search) {
				...
				queryResults = queryResults.slice( (pageNo-1)*recordsperpage , pageNo*recordsperpage );
			}
			query = null;
			cmtypes = null;
			retresult[ts.p.localReader.total] = totalpages;
			retresult[ts.p.localReader.pageNo] = pageNo;
		...
		updatepager = function(rn, dnd) {
			...
			base = parseInt(ts.p.pageNo,10)-1;
			if(base < 0) { base = 0; }
			...
			if (ts.p.scroll) {
				...
				if (rh) {
					...
					if (ts.grid.bDiv.scrollTop === 0 && ts.p.pageNo > 1) {
						ts.grid.bDiv.scrollTop = ts.p.rowNum * (ts.p.pageNo - 1) * rh;
					}
				}
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					...
					$('.ui-pg-input',pgboxes).val(ts.p.pageNo);
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				if(ts.p.pageNo <=0) { ts.p.pageNo = Math.min(1,ts.p.lastpage); }
				if(pN.search !== null) {prm[pN.search] = ts.p.search;} if(pN.nd !== null) {prm[pN.nd] = new Date().getTime();}
				if(pN.rows !== null) {prm[pN.rows]= ts.p.rowNum;} if(pN.pageNo !== null) {prm[pN.pageNo]= ts.p.pageNo;}
		...
		setPager = function (pgid, tp){
			...
			$('.ui-pg-selbox',"#"+pgcnt).bind('change',function() {
				...
				ts.p.pageNo = Math.round(ts.p.rowNum*(ts.p.pageNo-1)/this.value-0.5)+1;
			...
			if(ts.p.pgbuttons===true) {
				...
				$("#first"+$.jgrid.jqID(tp)+", #prev"+$.jgrid.jqID(tp)+", #next"+$.jgrid.jqID(tp)+", #last"+$.jgrid.jqID(tp)).click( function() {
					...
					if( this.id === 'first'+tp && fp ) { ts.p.pageNo=1; selclick=true;}
					if( this.id === 'prev'+tp && pp) { ts.p.pageNo=(cp-1); selclick=true;}
					if( this.id === 'next'+tp && np) { ts.p.pageNo=(cp+1); selclick=true;}
					if( this.id === 'last'+tp && lp) { ts.p.pageNo=last; selclick=true;}
					if(selclick) {
						populate();
					}
					return false;
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('keypress','input.ui-pg-input', function(e) {
					...
					if(key === 13) {
						...
						ts.p.pageNo = ($(this).val()>0) ? $(this).val():ts.p.pageNo;
						populate();
						return false;
					}
					return this;
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if (opts.pageNo) {
				...
				if (pageNo > ts.p.lastpage) { pageNo = ts.p.lastpage; }
				if (pageNo < 1) { pageNo = 1; }
				ts.p.pageNo = pageNo;
				if (ts.grid.prevRowHeight) {
					ts.grid.bDiv.scrollTop = (pageNo - 1) * ts.grid.prevRowHeight * ts.p.rowNum;
				} else {
					ts.grid.bDiv.scrollTop = 0;
				}
			}
			if (ts.grid.prevRowHeight && ts.p.scroll && opts.pageNo === undefined) {
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$("#gbox_"+$.jgrid.jqID($t.p.id)).css("width",nwidth+1+"px");
			$("#gview_"+$.jgrid.jqID($t.p.id)).css("width",nwidth+1+"px");
			$($t.grid.bDiv).css("width",nwidth+1 +"px");
			$($t.grid.hDiv).css("width",nwidth+1+"px");
			//console.log($t);
			//$t.width(nwidth-1);
			//$($t.grid.bDiv).css("width","1034px");
			//$($t.grid.hDiv).css("width","100%");
			if($t.p.pager ) {
	...
	clearGridData : function(clearfooter) {
		return this.each(function(){
			...
			$t.p.records = 0;$t.p.pageNo=1;$t.p.lastpage=0;$t.p.reccount=0;
			$t.p.data = []; $t.p._index = {};
			$t.updatepager(true,false);
		});
	},
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			triggerToolbar = function() {
				...
				if(!bsr) { $($t).jqGrid("setGridParam",{search:sd}).trigger("reloadGrid",[{pageNo:1}]); }
			...
			clearToolbar = function(trigger){
				...
				if(!bcv) {
					if(trigger) {
						$($t).jqGrid("setGridParam",{search:sd}).trigger("reloadGrid",[{pageNo:1}]);
					}
				}
				if(saveurl) {$($t).jqGrid("setGridParam",{url:saveurl});}
...
$.jgrid.extend({
	...
	groupingRender : function (grdata, colspans, pageNo, rn ) {
		return this.each(function(){
			var $t = this,
			grp = $t.p.groupingView,
			...
			$.each(grp.groups,function(i,n){
				if(grp._locgr) {
					if( !(n.startRow +n.cnt > (pageNo-1)*rn && n.startRow < pageNo*rn)) {
						return true;
					}
				}
				toEnd++;
				...
				opts.update = function (ev,ui) {
					...
					if($t.p.rownumbers === true) {
						$("td.jqgrid-rownum",$t.rows).each(function( i ){
							$(this).html( i+1+(parseInt($t.p.pageNo,10)-1)*parseInt($t.p.rowNum,10) );
						});
					}
					if(opts._update_) {
